...
export default function PatientRegister() {
  ...
  function update(field: string, value: string) {
    if (field === 'pin') value = value.replace(/\D/g, '').slice(0, 5);
  ...
  return (
    <div className="hospital-page flex items-center justify-center px-4 py-10">
      <motion.div className="w-full max-w-md" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
        <Link href="/hospital" className="inline-flex items-center gap-2 text-sm text-[#64748B] hover:text-[#1E293B] transition-colors mb-8">
        ...
        <div className="hospital-card p-8">
          <div className="text-center mb-8">
            ...
            <p className="text-sm text-[#64748B] mt-1">Create your account to book appointments</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <input type="text" placeholder="Enter your full name" required
                className="hospital-input text-base py-3.5" value={form.name}
                onChange={(e) => update('name', e.target.value)} />
            </div>
            <div>
              <label className="hospital-label">Phone Number</label>
              <input type="tel" inputMode="numeric" placeholder="Enter your phone number" required
                className="hospital-input text-base py-3.5" value={form.phone}
                onChange={(e) => update('phone', e.target.value)} />
            </div>
            <div>
              <label className="hospital-label">PIN Code</label>
              <input type="password" inputMode="numeric" placeholder="Create a 5-digit PIN" required maxLength={5}
                className="hospital-input text-base py-3.5 tracking-[0.5em] text-center" value={form.pin}
                onChange={(e) => update('pin', e.target.value)} />
              <p className="text-xs text-[#94A3B8] mt-1.5">Use this PIN to sign in later</p>
            </div>

            {error && (
              <motion.p className="text-sm text-[#EF4444] bg-[#FEF2F2] border border-[#FECACA] rounded-lg px-4 py-3"
          ...
          <p className="mt-6 text-center text-sm text-[#64748B]">
            ...
            <Link href="/hospital/login" className="text-[#8B5CF6] font-medium hover:underline">Sign in</Link>
          </p>
        </div>
      </motion.div>
    </div>
