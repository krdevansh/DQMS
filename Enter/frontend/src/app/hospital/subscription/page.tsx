...
import {
  ...
  Upload, Copy, ArrowLeft, LogOut, IndianRupee,
} from 'lucide-react';
...
export default function HospitalSubscriptionPage() {
  ...
  useEffect(() => {
    ...
    const user = getUser();
    if (!token || !user || user.role !== 'hospital_admin') {
      router.push('/');
      return;
    }
    fetchStatus();
      ...
      <nav className="sticky top-0 z-40 bg-[#080810]/90 backdrop-blur-xl border-b border-white/5">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button onClick={() => router.push('/hospital/dashboard')} className="p-2 rounded-xl bg-[#111118] border border-white/8 text-slate-500 hover:text-white transition-colors">
              <ArrowLeft className="w-4 h-4" />
            </button>
            <Building2 className="w-5 h-5 text-rose-400" />
            <span className="font-bold">Hospital Subscription</span>
          </div>
        ...
        {pageLoading ? (
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-[#111118] border border-white/5 rounded-2xl p-6 sm:p-8">
            <div className="text-center mb-6">
              ...
              <h1 className="text-2xl font-bold">Hospital Subscription</h1>
            ...
            {(!currentSub || currentSub.status === 'rejected' || currentSub.status === 'expired') && (
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <button type="submit" disabled={loading} className="w-full py-3 bg-gradient-to-r from-rose-500 to-pink-600 text-white font-bold rounded-xl hover:opacity-90 transition-opacity disabled:opacity-50 flex items-center justify-center gap-2 text-sm">
