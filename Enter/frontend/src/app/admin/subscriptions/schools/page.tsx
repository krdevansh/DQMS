...
import { GraduationCap, CheckCircle, XCircle, RefreshCw, LogOut, ArrowLeft, ExternalLink, Clock, Phone, IndianRupee } from 'lucide-react';
...
export default function AdminSchoolSubscriptions() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    const { data } = await adminApi.get<{ subscriptions: Subscription[] }>('/admin/subscriptions/schools');
    ...
    <div className="min-h-screen bg-[#080810] text-white">
      ...
      <nav className="sticky top-0 z-40 bg-[#080810]/90 backdrop-blur-xl border-b border-white/5">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="bg-gradient-to-br from-amber-500 to-orange-600 p-2 rounded-xl shadow-lg shadow-amber-500/20"><GraduationCap className="w-5 h-5 text-white" /></div>
            <div><span className="font-bold">School Subscriptions</span><p className="text-[10px] text-slate-500">Manage school subscription requests</p></div>
          </div>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        ...
        {loading ? (
          ...
          <div className="bg-[#111118] border border-white/5 rounded-2xl p-12 text-center"><GraduationCap className="w-10 h-10 text-slate-700 mx-auto mb-3" /><p className="text-slate-500">No subscriptions yet</p></div>
        ) : (
          <div className="space-y-3">
            {subscriptions.map((s, i) => (
              <motion.div key={s._id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }} className="bg-[#111118] border border-white/5 rounded-2xl p-4 sm:p-5 hover:border-white/10 transition-colors">
                <div className="flex flex-col sm:flex-row sm:items-start gap-4">
                  <div className="flex items-start gap-3 flex-1 min-w-0">
                    <div className="w-10 h-10 bg-gradient-to-br from-amber-500/20 to-orange-500/20 rounded-xl flex items-center justify-center flex-shrink-0"><GraduationCap className="w-5 h-5 text-amber-400" /></div>
